...
function mlt_db(array $settings): ?PDO {
    ...
    try {
        ...
        $pdo->exec('CREATE TABLE IF NOT EXISTS mlt_orders (id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY, reference_code VARCHAR(80) NOT NULL UNIQUE, status VARCHAR(24) NOT NULL DEFAULT "requested", paypal_order_id VARCHAR(128) NULL, paypal_capture_id VARCHAR(128) NULL, amount DECIMAL(12,2) NULL, currency CHAR(3) NOT NULL DEFAULT "EUR", first_name VARCHAR(80) NOT NULL, last_name VARCHAR(80) NOT NULL, customer_email VARCHAR(160) NOT NULL, phone VARCHAR(80) NULL, collection_name VARCHAR(120) NULL, vehicle_name VARCHAR(120) NULL, country_name VARCHAR(80) NULL, guests VARCHAR(40) NULL, travel_days SMALLINT NULL, arrival_date DATE NULL, departure_date DATE NULL, route_json TEXT NULL, extras_json TEXT NULL, notes TEXT NULL, created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP, paid_at DATETIME NULL, updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4');
        return $pdo;
...
function mlt_create_order(PDO $db, string $reference, array $body, array $customer): void {
    try {
        ...
        $statement = $db->prepare('INSERT INTO mlt_orders (reference_code, first_name, last_name, customer_email, phone, collection_name, vehicle_name, country_name, guests, travel_days, arrival_date, departure_date, route_json, extras_json, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
        $statement->execute([$reference, $customer['firstName'], $customer['lastName'], $customer['email'], $customer['phone'], $body['collection'] ?? null, $body['vehicle'] ?? null, $body['country'] ?? null, $body['guests'] ?? null, (int)($body['days'] ?? 0) ?: null, $body['arrival'] ?? null, $body['departure'] ?? null, json_encode($route, JSON_UNESCAPED_UNICODE), json_encode($extras, JSON_UNESCAPED_UNICODE), $body['notes'] ?? null]);
