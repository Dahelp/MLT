...
export function FleetDetail({ vehicle }: { vehicle: Vehicle }) {
  ...
  return <main className={`fleet-detail fleet-detail-${vehicle.id}`}>
    ...
    <section className="detail-cta fleet-detail-cta"><p className="section-label">MLT Fleet / {vehicle.number}</p><h2>{copy.labels.request}</h2><a className="primary-button" href="/#contact">{copy.labels.request}<span>↗</span></a></section>
