...
export function FleetDetail({ vehicle }: { vehicle: Vehicle }) {
  ...
  return <main className={`fleet-detail fleet-detail-${vehicle.id}`}>
    <header className="detail-nav"><a className="brand" href="/"><img className="brand-logo" src="/mlt-logo.svg" alt="MLT — Move. Live. Travel." /></a><a href="/#fleet">← {copy.labels.back}</a><div className="language-switch"><button className={locale === "en" ? "active" : ""} onClick={() => changeLocale("en")}>EN</button><button className={locale === "de" ? "active" : ""} onClick={() => changeLocale("de")}>DE</button></div></header>
    ...
    <footer className="detail-footer"><div className="brand"><img className="brand-logo" src="/mlt-logo.svg" alt="MLT — Move. Live. Travel." /></div><p>{copy.labels.concept}</p><a href="/">mlt.travel</a></footer>
  </main>;
}
