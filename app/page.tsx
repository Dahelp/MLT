  ...
  return <main className="home-light">
    ...
    <section className="light-collections" id="collections">
      ...
      <SwiperCarousel className="collection-rail" modules={[EffectCoverflow, Keyboard]} effect="coverflow" initialSlide={carouselMiddleStart + 4} centeredSlides centeredSlidesBounds={false} slidesPerView="auto" speed={850} grabCursor keyboard={{ enabled: true }} coverflowEffect={{ rotate: 0, stretch: 22, depth: 145, modifier: 1, slideShadows: false }} onSwiper={(instance) => { carousel.current = instance; setActiveCollection(instance.activeIndex % collections.length); }} onSlideChange={(instance) => setActiveCollection(instance.activeIndex % collections.length)} onSlideChangeTransitionEnd={(instance) => { if (instance.activeIndex < collections.length * 2 || instance.activeIndex >= collections.length * (carouselCopies - 2)) instance.slideTo(carouselMiddleStart + (instance.activeIndex % collections.length), 0, false); }} aria-label="MLT collections">
        {carouselCollections.map((item, index) => { const localizedItem = locale === "ru" ? russianCollections[index % collections.length] : item; return <SwiperSlide className="collection-slide" key={`${item.id}-${index}`}><a className="collection-card-link" href={`/collections/${item.id}`} aria-label={locale === "ru" ? `Открыть коллекцию MLT «${localizedItem.name}»` : `Open MLT ${item.name} Collection`}><article className="light-collection-card">
    ...
    <section className="light-map" id="smart-map">
      ...
      <div className="map-copy"><p className="light-section-label">04 / {t.mapLabel}</p><h2>{t.mapTitle}</h2><p>{t.mapCopy}</p><div className="map-stats"><div><strong>30+</strong><span>{locale === "ru" ? "отобранных мест" : "curated places"}</span></div><div><strong>3</strong><span>{locale === "ru" ? "страны на старте" : "countries at launch"}</span></div></div><a className="bronze-button" href="/plan">{t.openMap}<span>↗</span></a></div>
    </section>
    ...
    <footer className="light-footer"><div className="light-footer-logo"><img src="/mlt-logo-bronze.png" alt="MLT" /><p>{t.footer}</p></div><div><strong>{locale === "ru" ? "Разделы" : "Explore"}</strong><a href="#collections">{t.nav[0]}</a><a href="#experiences">{t.nav[1]}</a><a href="/plan">{t.nav[2]}</a></div><div><strong>{locale === "ru" ? "Документы" : "Legal"}</strong><a href="/legal/imprint">{locale === "ru" ? "Выходные данные" : "Impressum"}</a><a href="/legal/privacy">{locale === "ru" ? "Конфиденциальность" : "Datenschutz"}</a><a href="/legal/terms">{locale === "ru" ? "Условия" : "AGB"}</a></div><div><strong>{locale === "ru" ? "Контакты" : "Contact"}</strong><a href="mailto:concierge@mlt-travel.com">concierge@mlt-travel.com</a><a href="tel:+4917632523799">+49 176 325 23 799</a></div><small>© 2026 MLT Maschinenhandel GmbH Import-Export</small></footer>
