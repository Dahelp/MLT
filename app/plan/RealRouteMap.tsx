...
export default function RealRouteMap({ selected, country, onToggle }: { selected: string[]; country: string; onToggle: (id: string) => void }) {
  ...
  useEffect(() => {
    ...
    markersRef.current = mapPoints.map((point, index) => { const active = selected.includes(point.id); const button = document.createElement("button"); button.className = `real-map-marker${active ? " selected" : ""}${country !== "All" && country !== point.country ? " muted" : ""}`; button.type = "button"; button.setAttribute("aria-label", `${active ? "Remove" : "Add"} ${point.name}`); button.setAttribute("aria-pressed", String(active)); button.innerHTML = `<span>${active ? "✓" : index + 1}</span><b>${point.name}</b>`; button.onclick = () => toggleRef.current(point.id); return new maplibregl.Marker({ element: button, anchor: "center" }).setLngLat(coordinates[point.id]).addTo(map); });
    const update = () => { const source = map.getSource("route") as maplibregl.GeoJSONSource | undefined; if (source) source.setData({ type: "Feature", properties: {}, geometry: { type: "LineString", coordinates: selected.map((id) => coordinates[id]) } }); };
    if (map.isStyleLoaded()) update(); else map.once("load", update);
  }, [selected, country]);
  return <div ref={container} className="real-route-map" aria-label="Interactive map of MLT destinations">
    {status !== "ready" && <div className={`map-status ${status}`} role="status">{status === "error" ? "Map tiles are temporarily unavailable. Your selected places remain saved." : "Loading curated map…"}</div>}
  </div>;
}
