...
export default function RealRouteMap({ selected, country, onToggle, locale = "en", className = "" }: { selected: string[]; country: string; onToggle: (id: string) => void; locale?: Locale; className?: string }) {
  ...
  useEffect(() => {
    ...
    map.on("load", () => {
      for (const name of countryFiles) { map.addSource(`country-${name}`, { type: "geojson", data: `/data/countries/${name}.geojson` }); map.addLayer({ id: `country-${name}-fill`, type: "fill", source: `country-${name}`, paint: { "fill-color": "#b66f31", "fill-opacity": 0.1 } }); map.addLayer({ id: `country-${name}-line`, type: "line", source: `country-${name}`, paint: { "line-color": "#a45f28", "line-width": 2.2, "line-opacity": 0.9 } }); }
