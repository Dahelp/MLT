  ...
  return <main className="home-light">
    ...
    <section className="light-hero" id="top">
      ...
      <div className="light-hero-copy">
        ...
        <p>{t.hero}</p>
        <div className="light-hero-buttons"><a className="bronze-button" href="#collections">{t.choose}<span>↗</span></a><a className="text-button" href={localPath("/plan")}>{t.route}<span>→</span></a></div>
      </div>
    ...
    <section className="light-map" id="smart-map">
      ...
      <div className="map-copy"><p className="light-section-label">04 / {t.mapLabel}</p><h2>{t.mapTitle}</h2><p>{t.mapCopy}</p><div className="smart-map-selection"><small>{locale === "ru" ? "Выбранные места" : locale === "de" ? "Ausgewählte Orte" : "Selected places"}</small><div>{mapSelection.length ? mapSelection.map((id, index) => { const point = mapPoints.find((item) => item.id === id); return point && <button key={id} onClick={() => toggleMapPoint(id)}><span>{index + 1}</span>{point.name}<b>×</b></button>; }) : <p>{locale === "ru" ? "Выберите точки на карте" : locale === "de" ? "Wählen Sie Orte auf der Karte" : "Choose places on the map"}</p>}</div></div><div className="map-stats"><div><strong>30+</strong><span>{locale === "ru" ? "отобранных мест" : "curated places"}</span></div><div><strong>3</strong><span>{locale === "ru" ? "страны на старте" : "countries at launch"}</span></div></div><a className="bronze-button" href={`${localPath("/plan")}?route=${mapSelection.join(",")}`}>{t.openMap}<span>↗</span></a></div>
    </section>
