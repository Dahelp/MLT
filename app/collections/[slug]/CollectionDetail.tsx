...
export function CollectionDetail({ collection }: { collection: Collection }) {
  ...
  return <main className={`detail-page detail-${collection.id}`}>
    ...
    <section className="detail-hero">
      ...
      <div className="detail-hero-copy"><p className="section-label">{item.eyebrow}</p><h1>{item.name}<br /><em>Collection</em></h1><p>{copy.manifesto}</p><a className="primary-button" href={`/#contact?collection=${item.id}`}>{ui.explore}<span>↗</span></a></div>
    ...
    <section className="detail-cta"><p className="section-label">MLT {item.name}</p><h2>{copy.cta}</h2><a className="primary-button" href={`/#contact?collection=${item.id}`}>{ui.explore}<span>↗</span></a></section>
